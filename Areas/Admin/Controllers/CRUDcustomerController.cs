    ...
    public class CRUDcustomerController : Controller
    {
        ...
        public JsonResult DsCustomer()
        {
            try
            {
                ...
                return Json(new { code = 200, dsCustomer = dsCustomer, msg = "Lấy thành công danh sách Khách hàng!" }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                return Json(new { code = 500, msg = "Lấy thất bại danh sách Khách hàng: " + ex.Message, JsonRequestBehavior.AllowGet });
            }
        }
        [HttpPost]
