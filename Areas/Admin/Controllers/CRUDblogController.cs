...
namespace ShopOnline.Areas.Admin.Controllers
{
    ...
    public class CRUDblogController : Controller
    {
        ...
        public ActionResult Create(Article article, HttpPostedFileBase uploadFile)
        {
            try
            {
                ...
                if (extension.ToLower() == ".jpg" || extension.ToLower() == ".jpeg" || extension.ToLower() == ".png")
                {
                    if (uploadFile == null)
                    {
                        ModelState.AddModelError("", "Lỗi khi tải file.");
                    }
                    else
                    {
                        ...
                        if (db.SaveChanges() > 0)
                        {
                            ...
                            ModelState.Clear();
                            TempData["msgCreate"] = "Thêm tin tức thành công!";
        ...
        public ActionResult Edit(Article article, HttpPostedFileBase uploadFile, FormCollection collection)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    if (uploadFile != null)
                    {
                        ...
                        if (db.SaveChanges() > 0)
                        {
                            TempData["msgEdit"] = "Sản phẩm chỉnh sửa thành công có ID: " + article.articleId;
                            uploadFile.SaveAs(path);
            ...
            {
                TempData["msgEditFailed"] = "Chỉnh sửa thất bại! " + ex.Message;
                return RedirectToAction("Index");
            }
        }
        //DELETE
