    ...
    public class CRUDcontactController : Controller
    {
        ...
        public JsonResult DsContact()
        {
            try
            {
                ...
                return Json(new { code = 200, dsContact = dsContact, msg = "Lấy thành công danh sách liên hệ!" }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                return Json(new { code = 500, msg = "Lấy thất bai danh sách liên hệ: " + ex.Message, JsonRequestBehavior.AllowGet });
            }
        }
        [HttpPost]
